---
title: "A Practial Guide to the PEB"
...
dataset with observed marker levels from a population of healthy individuls (the
'prior population' here on), at least some of whome have marker levels taken from
...
an MVU estimate of the the `s2`.  Note that excess variation attributable to the
process of extimateing the within person means (`m_i`) is subtraced from the
sample variance (`var(m_i)`).

To illustrate the processe of estimating these paraemters, a random prior population 
...
The function `PEBparams()` calculates *intital* estimates for `mu`, `s2`, and `t2`
...
Note that in addition to each of the required paremters ,
...
The above estiamate of `s2` is indeed the estimate for `s2` returned by
...
Unfortunatly, closed form MVU estimates of `mu` and `s2` are not available.
...
numbers of prior observations per person as well as relative magnituds of `t2` and `s2`.
...
The iterative prcoess begins with a naive estiate of `mu` and `t2`,
...
`PEBparams(...,iterations=10)` could have been specifided with no effect on the
estimates.
...
with specficity 95%, a threshold can be calculated using `threshold = qpeb(...)`, 
and the current biomarker value (`y_current`) is condered to be elvated, if 
...
Note that inistead of passing each parameter explicitly, the `params` object
...
An alternative to calculating the a threshold and deteriming if the current
biomarker value exceeds taht threshold is to calculate the probability of the current
value conditional on her history and the distribution of the biomarker in the
prior population by using `ppeb(...)` and testing wether the probability exceeds the
...
previous biomarker level from the same individual were calculated explicity and
...
insufficiently elevated marker levels are not indicitive of a given outcome
...
Devaitions from the expected FPR [ = 1 - specificity] may result from random
variation in the estimates of the parmeters `mu`, `s2`, and `mu`, as well as
from failure of the biomarker to follow a normal distributed within-individuals,
between-individuals, or both.  In practice it is often nessecary to apply a
transformation (such as the log-transformation) to the biomarker prior to
applying the PEB algorothm.  Howeve, failure of the biomarker to meet
assumtptions on it's distribution is beyond the topic of this discussion.
...
only for random deviations in the estiamtes of the parameters `t2` and `s2` and
...
the conficence intervals may go from narrow to wide, or vice versa with
increasing numbers of prior observations in the individual's history, because
the total variation and within person variation estiamtes are statistically
...
when calculationg parameters for the PEB algorithm. 

# Comparison of the 'iterative' and 'ICC1' methods

The parameters `s2` and `t2` are related to the Interclass Correlation
Coeffiect (beta) by the equation \deqn{beta = t2/(s2 + t2)}, which can be estimated
...
differences between estimates `s2` adn `t2` are dwarfed by their standarad
devation (Tables 1 and 2, below).  Furthermore, the standard deviations of each
paramerer is similar under both methods, which suggests that neither method is
...
The tables below compare the differneces (1) differences betwen the mean
parameter estimate and it true value and (2) the standard devation of each
...
each of whome contribures 2,3, or 4 samples with equal probabilities, with
serveral values of the ICC (beta) parameters.

Notice that estimates for `s2` and `V` are virtually unchanged with after the
second iteration (Tables 2 and 3), and because estiamtes of `s2` are unchanged
...
# Convenience functions for calculationg the SD and bias 
SD <- function(x,exp)
